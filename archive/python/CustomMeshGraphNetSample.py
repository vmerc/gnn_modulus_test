...
class MeshGraphNet(Module):
    ...
    def forward(
        ...
        common_edge_indices: List[torch.Tensor],
    ) -> Tensor:
        
        if isinstance(graph,List):
            assert(len(graph)==self.processor_size)

        edge_features = self.edge_encoder(edge_features)
        node_features = self.node_encoder(node_features)

        x = self.processor(node_features, edge_features, graph,common_edge_indices)
...
class MeshGraphNetProcessor(nn.Module):
    ...
    def __init__(
        ...
        layers = list(chain(*zip(self.edge_blocks, self.node_blocks)))

        self.processor_layers = nn.ModuleList(layers)
        self.num_processor_layers = self.processor_size
    ...
    def forward(
        ...
        for i,gra in enumerate(graph) : 
            ...
            edge_features, node_features = self.edge_blocks[i](edge_features,node_features,gra)

            edge_features, node_features = self.node_blocks[i](edge_features,node_features[:gra.num_dst_nodes()],gra)
       

            if i!= len(graph)-1 :
                common_edge_indice = common_edge_indices[i]
                edge_features = transfer_edge_features(gra,edge_features,graph[i+1],common_edge_indice)
...
def transfer_edge_features(current_block: DGLBlock, edge_features: torch.Tensor, next_block: DGLBlock, common_edge_indices: torch.Tensor) -> torch.Tensor:
    # Récupérer les indices des arêtes pour le bloc suivant
    src_next, dst_next = next_block.edges()

    # Trouver les caractéristiques des arêtes correspondantes pour le bloc suivant
    next_edge_features = torch.zeros((len(src_next), edge_features.size(1)), device=edge_features.device)
    next_edge_features[common_edge_indices] = edge_features[common_edge_indices]

    return next_edge_features
