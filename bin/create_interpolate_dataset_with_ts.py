...
def interpolate_data(data_list, X_coarse, triangulation, X_fine):
    """
    Interpole spatialement les champs nodaux (a_i, b_i) du maillage fin vers X_coarse.
    Le timestamp ts est recopié tel quel (pas d'interpolation temporelle ici).
    """
    ...
    for idx, sample in enumerate(data_list):
        ...
        a_i, b_i, ts = unpack_sample(sample)
        a_i = np.ascontiguousarray(a_i)
        b_i = np.ascontiguousarray(b_i)

        # Vérifications de cohérence
        assert a_i.shape[0] == X_fine.shape[0], f"Mismatch a_i vs X_fine at sample {idx+1}"
        assert b_i.shape[0] == X_fine.shape[0], f"Mismatch b_i vs X_fine at sample {idx+1}"

        # Interpolation multi-colonnes
        interpolator_a = LinearNDInterpolator(triangulation, a_i)
        a_i_interp = interpolator_a(X_coarse)

        interpolator_b = LinearNDInterpolator(triangulation, b_i)
        b_i_interp = interpolator_b(X_coarse)

        # Gestion extrapolation / points hors enveloppe
        a_i_interp = np.nan_to_num(a_i_interp, nan=0.0)
        b_i_interp = np.nan_to_num(b_i_interp, nan=0.0)

        # Conserver le format de sortie selon présence de ts
        if ts is None:
            interpolated_results.append((a_i_interp, b_i_interp))
        else:
            interpolated_results.append((a_i_interp, b_i_interp, int(ts)))
