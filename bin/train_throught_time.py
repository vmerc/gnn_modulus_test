...
class MGNTrainer:
    def __init__(self, cfg: DictConfig, rank_zero_logger: RankZeroLoggingWrapper):
        ...
        self.h_u_v_i_0_mean = torch.tensor([
            ...
            node_stats['u'].item(),
            node_stats['v'].item()
        ], dtype=torch.float32).to(self.dist.device)
    ...
    def forward(self, batch):
        ...
        # Initialisation du premier graphique
        graph = batch[0].to(self.dist.device)
        ...
        # Boucle à travers la séquence
        for t in range(1, num_steps):
            next_graph = batch[t].to(self.dist.device)
            # Modification du graphique actuel
    ...
    def modify_graph(self, graph_0, pred_0, graph_1):
        ...
        # Do not detach pred_0 to allow gradients to flow back
