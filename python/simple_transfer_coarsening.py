...
class CoarseMesh:
    ...
    topo_preserve_edge_mask: np.ndarray | None = None
    topo_cut_edge_mask: np.ndarray | None = None
    used_dz_min: float | None = None
    used_slope_min: float | None = None
    used_cut_dz_min: float | None = None
    used_cut_slope_min: float | None = None
...
def build_simple_coarse_mesh(
    ...
    dz_min: float | None = None,
    slope_min: float | None = None,
    cut_dz_min: float | None = None,
    cut_slope_min: float | None = None,
    cut_dz_quantile: float = 0.995,
    cut_slope_quantile: float = 0.995,
    weights: np.ndarray | None = None,
) -> CoarseMesh:
    """
    ...
    if cut_dz_min is None:
        cut_dz_floor = used_dz_min
    else:
        cut_dz_floor = max(float(cut_dz_min), float(used_dz_min))

    if cut_slope_min is None:
        cut_slope_floor = used_slope_min
    else:
        cut_slope_floor = max(float(cut_slope_min), float(used_slope_min))

    topo_cut_edge_mask, used_cut_dz_min, used_cut_slope_min = sharp_topography_edges(
        xy=xy,
        edges=fine_edges,
        z=z,
        dz_min=None if cut_dz_min is None else cut_dz_floor,
        slope_min=None if cut_slope_min is None else cut_slope_floor,
        dz_quantile=cut_dz_quantile,
        slope_quantile=cut_slope_quantile,
        dz_floor=cut_dz_floor,
        slope_floor=cut_slope_floor,
    )

    u = fine_edges[:, 0]
    v = fine_edges[:, 1]
    boundary_change_edge_mask = boundary_type[u] != boundary_type[v]
    cut_edge_mask = topo_cut_edge_mask | boundary_change_edge_mask
    ...
    print(
        ...
        f"cut_dz_min={used_cut_dz_min:.3g}",
        f"cut_slope_min={used_cut_slope_min:.3g}",
        f"topo_nodes={int(topo_barrier_node_mask.sum())}",
        f"topo_preserve_edges={int(topo_preserve_edge_mask.sum())}",
        f"topo_cut_edges={int(topo_cut_edge_mask.sum())}",
    ...
    return CoarseMesh(
        ...
        topo_barrier_node_mask=topo_barrier_node_mask,
        preserve_node_mask=preserve_node_mask,
        topo_preserve_edge_mask=topo_preserve_edge_mask,
        topo_cut_edge_mask=topo_cut_edge_mask,
        used_dz_min=float(used_dz_min),
        used_slope_min=float(used_slope_min),
        used_cut_dz_min=float(used_cut_dz_min),
        used_cut_slope_min=float(used_cut_slope_min),
    )
